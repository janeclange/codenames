...
class Cluer:
		...
		with open("codewords_simplified.txt") as f: 
			...
			for i in range(len(codewords)):
				...
				for j in range(i+1, len(codewords)):
					...
					#print([w,w2])
					#print(clues)
					clues = self.g.guesser.filter_valid_words(clues, (w,w2))
					if clues:
						scored_clues = self.g.guesser.score_clues([w,w2],clues)
						self.clues[(w,w2)] = scored_clues[0][:1]
					else:
	...
	def evaluate_tup(self, word_tup, board):
		...
		for i in range(trials):
			...
			#print(self.clues[tuple(word_tup)][0])
			# print(self.clues[tuple(word_tup)])
			if (tuple(word_tup) in self.clues):
				if (self.clues[tuple(word_tup)]):
					#print(self.clues[tuple(word_tup)])
					clue = self.clues[tuple(word_tup)][0]
					# print(clue)
				else:
				...
				turn_counts.append(2)
				break 	# no clue for that pair, we should hint it as two singletons
			board_words_ordered, inner_prods = self.g.guesser.guess(clue, board, len(board))
			noise = np.random.normal(0,0.03,size=(len(board)))
			scores = (np.arcsin(inner_prods) / (np.pi/2)) + noise
			...
			if intended_set == guess_set:
				...
				#print("Two positive")
				turns = 1.25 	# effective turns: slightly more than 1, because you might have messed up your partition
				...
				turns = 1.5 	# effective turns: 1 turn actually passes, +0.5 penalty because you'll have to reclue the other word
			elif len(guess_set.intersection(set(self.bystanders))) == 2:
				#print("Two neutral")
				turns= 2 		# 1 turn actually passes, and you'll have to reclue both ally words
				...
				turns = 2.5 	# effective turns: 1 turn passes, +1 for negative word, and you'll also have to reclue the other ally word
				...
				turns = 3 	# effective turns: 1 turn passes, +1 for negative word, and you'll have to reclue both ally words
	...
	def clue(self):
		...
		for i in range(len(partitions)):
			...
			for tup in partition:
				...
				tup_score = self.evaluate_tup(word_tup, board)
		...
		for i in range(len(best_partition)):
			...
			word_tup = [remaining_blue_words[t] for t in tup]
			# print(word_tup)
			tup_scores.append(self.evaluate_tup(word_tup, board))
		...
		if (tuple(word_best_tup) in self.clues):
			if (self.clues[tuple(word_best_tup)]) and (np.min(tup_scores) < 2): # use pair clue only if it exists and doesn't suck
				clue = self.clues[tuple(word_best_tup)][0]
				n_target = 2
			else:
...
if __name__=="__main__":
	c = Cluer(assassin=["bell"], blue_words=["bridge","jupiter"], red_words=["egypt","greece"],bystanders=["africa","air"])
	c.load_clues()
	print( c.clue() )
	# c.precompute()
	# c.save_clues()
